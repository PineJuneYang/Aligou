...
import com.alg.ailigou.pages.personal.myorder.holder.PrePaymentViewHolder;
...
public class PrePaymentFragmentAdapter extends BaseHeadAndFootAdapter {
    private int position;


    private List<OrderDetailsDataModel> commodityModels = new ArrayList<>();

    public PrePaymentFragmentAdapter(List datas, Context context, int position) {
        super(datas, context);
    ...
    protected MyBaseViewHolder onNormalViewHolder(ViewGroup parent, int type, OnItemClickListener listener) {
        ...
        return new PrePaymentViewHolder(inflate, listener, getOnClickListener(), position,mContext);
    }

    @Override
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof PrePaymentViewHolder) {
            PrePaymentViewHolder prePaymentViewHolder = (PrePaymentViewHolder) holder;
            prePaymentViewHolder.setData(commodityModels.get(position));
        }
    }
}
