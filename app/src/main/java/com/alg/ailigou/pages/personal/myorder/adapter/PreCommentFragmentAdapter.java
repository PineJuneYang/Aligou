...
import com.alg.ailigou.pages.personal.myorder.holder.PreCommentViewHolder;
...
public class PreCommentFragmentAdapter extends BaseHeadAndFootAdapter {
    private int position;


    private List<OrderDetailsDataModel> commodityModels = new ArrayList<>();

    public PreCommentFragmentAdapter(List datas, Context context, int position) {
        super(datas, context);
    ...
    protected MyBaseViewHolder onNormalViewHolder(ViewGroup parent, int type, OnItemClickListener listener) {
        ...
        return new PreCommentViewHolder(inflate, listener, getOnClickListener(), position,mContext);
    }

    @Override
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof PreCommentViewHolder) {
            PreCommentViewHolder preCommentViewHolder = (PreCommentViewHolder) holder;
            preCommentViewHolder.setData(commodityModels.get(position));
        }
    }
}
