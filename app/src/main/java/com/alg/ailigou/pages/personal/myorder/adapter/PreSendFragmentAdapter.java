...
import com.alg.ailigou.pages.personal.myorder.holder.PreSendViewHolder;
...
public class PreSendFragmentAdapter extends BaseHeadAndFootAdapter {
    private int position;


    private List<OrderDetailsDataModel> commodityModels = new ArrayList<>();

    public PreSendFragmentAdapter(List datas, Context context, int position) {
        super(datas, context);
    ...
    protected MyBaseViewHolder onNormalViewHolder(ViewGroup parent, int type, OnItemClickListener listener) {
        ...
        return new PreSendViewHolder(inflate, listener, getOnClickListener(), position,mContext);
    }

    @Override
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof PreSendViewHolder) {
            PreSendViewHolder preSendViewHolder = (PreSendViewHolder) holder;
            preSendViewHolder.setData(commodityModels.get(position));
        }
    }
}
